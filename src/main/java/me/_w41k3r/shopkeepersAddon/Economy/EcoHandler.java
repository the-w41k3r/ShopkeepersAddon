...
public class EcoHandler {
        ...
        if((e.getCurrentItem() == null || !(e.getCurrentItem().isSimilar(e.getCursor())))
            ...
            e.getClickedInventory().setItem(e.getSlot()+18,
                    InvUtils.ItemBuilder(Material.valueOf(
                            Main.getInstance().messages.getString("Currency-Item.Material")), 1,
                            Main.getInstance().messages.getString("Currency-Item.Name-Format").replace("[amount]",
                                    Main.getInstance().vaultHook.formattedMoney(0.0)),
                            Main.getInstance().messages.getStringList("Currency-Item.Lore"),
            "ItemPrice", 0.0, Main.getInstance().messages.getInt("Currency-Item.CustomModelData")));
        }
    ...
    public static void onPriceItemClick(Player p, ItemStack clickedItem, Shopkeeper shopkeeper){
        ...
        p.sendMessage(Main.getInstance().messages.getString("Price-Input-Request"));
    ...
    public static void onPriceSuccess(Player p, Shopkeeper shopkeeper) {
        p.closeInventory();
        shopkeeper.openEditorWindow(p);
        p.sendMessage(Main.getInstance().messages.getString("Price-Set-Successfully"));
    }

    public static void onPriceInput(Player p, String priceInput, ItemStack clickedItem, Shopkeeper shopkeeper){
        try {
            double price = Double.parseDouble(priceInput);
            if(price>Main.getInstance().getConfig().getDouble("EconomyHook.SellingPriceLimit")){
//                onPriceCancel(p, "§cYour trade setup was cancelled due to invalid price input. Highest price limit is "
//                        +String.format("%.2f",Main.getInstance().getConfig().getDouble("SellingPriceLimit")), shopkeeper);
                onPriceCancel(p, Main.getInstance().messages.getString("Price-Limit-Reached")
                        .replace("[maxPrice]", String.format("%.2f", Main.getInstance().getConfig().getDouble("EconomyHook.SellingPriceLimit"))), shopkeeper);
                return;
            }
            ItemMeta meta = clickedItem.getItemMeta();
//            meta.setDisplayName("Price "+String.format("%.2f",price));
            meta.setDisplayName(Main.getInstance().messages.getString("Currency-Item.Name-Format").replace("[amount]",
                    Main.getInstance().vaultHook.formattedMoney(price)));
            meta.getPersistentDataContainer().set(new NamespacedKey(Main.getInstance(), "ItemPrice"), PersistentDataType.DOUBLE, price);
            clickedItem.setItemMeta(meta);
            onPriceSuccess(p, shopkeeper);

        }catch (Exception e){
            onPriceCancel(p, Main.getInstance().messages.getString("Invalid-Price-Input"), shopkeeper);
        }
    }
    ...
    public static void onTradeSelect(TradeSelectEvent e){
        ...
        if (InvUtils.hasPersistentData("ItemPrice", item1, PersistentDataType.DOUBLE)) {
            ...
            if (Main.getInstance().vaultHook.hasMoney(p.getName(), price)) {
...
//            System.out.println("MERCHANT NULL");
        ...
        if(p.getInventory().firstEmpty()==-1){
            p.sendMessage(Main.getInstance().messages.getString("Player-Inventory-Full"));
            p.closeInventory();
            return;
        }
        ...
        if((InvUtils.hasPersistentData("ItemPrice",item1,PersistentDataType.DOUBLE)
            ...
            if(keeper instanceof PlayerShopkeeper){
                ...
                if(!c1.getInventory().containsAtLeast(merchantInv.getMerchant().getRecipe(merchantInv.getSelectedRecipeIndex()).getResult(), merchantInv.getMerchant().getRecipe(merchantInv.getSelectedRecipeIndex()).getResult().getAmount())) {
                    p.sendMessage(Main.getInstance().messages.getString("Shop-Out-Of-Stock"));
                    return;
                }
                if (item2 != null) {
                    ...
                    if(!left.isEmpty()){
                        ...
                        resultClone.setAmount(addedAmount);
                        c1.getInventory().removeItem(resultClone);
                        p.sendMessage(Main.getInstance().messages.getString("Supply-Chest-Is-Full"));
                        p.closeInventory();
                        return;
                    }
            ...
            if(!Main.getInstance().vaultHook.hasMoney(p.getName(), price)) {
                p.closeInventory();
                p.sendMessage(Main.getInstance().messages.getString("Player-Out-Of-Money"));
                return;
            }
            Main.getInstance().vaultHook.takeMoney(p.getName(), price);
            if(!Main.getInstance().vaultHook.hasMoney(p.getName(), price)){
            ...
            if(keeper instanceof PlayerShopkeeper){
                ...
                Main.getInstance().vaultHook.giveMoney(ps.getOwnerName(), price);
...
//                InvUtils.removeItems(e.getTradingRecipe().getResultItem().getType(), e.getTradingRecipe().getResultItem().getAmount(), c.getBlockInventory());
                ...
                if(Bukkit.getServer().getOnlinePlayers().contains(ps.getOwner())){
                    ps.getOwner().sendMessage(Main.getInstance().messages.getString("Owner-Add-Money").replace("[amount]", String.format("%.2f", price)));
                }
                p.updateInventory();
            }
...
//                    System.out.println("REMOVED");
            ...
            if (keeper instanceof PlayerShopkeeper) {
                ...
                if (!Main.getInstance().vaultHook.hasMoney(ps.getOwnerName(), price)) {
                    p.closeInventory();
                    p.sendMessage(Main.getInstance().messages.getString("Shop-Out-Of-Money"));
                    return;
                }
                ...
                Main.getInstance().vaultHook.takeMoney(ps.getOwnerName(), price);
                if (Bukkit.getServer().getOnlinePlayers().contains(ps.getOwner())) {
//                    ps.getOwner().sendMessage("§a-" + String.format("%.2f", price));
                    ps.getOwner().sendMessage(Main.getInstance().messages.getString("Owner-Subtract-Money").replace("[amount]", String.format("%.2f", price)));
                }
...
//                e.getClickedInventory().setItem(1, null);
                Main.getInstance().vaultHook.giveMoney(p.getName(), price);
