...
public class EcoListeners implements Listener {
    ...
    public void onUIClick1(InventoryClickEvent e){
        ...
        if (ShopkeepersAPI.getUIRegistry().getUISession(p).getUIType() == EditorUIType.INSTANCE) {
            if (e.getClickedInventory() == e.getView().getTopInventory()) {
                if(Main.getInstance().getConfig().getBoolean("EconomyHook.BlockItemBasedTrades")) {
...
//                        e.getInventory().setItem(e.getSlot(), e.getCurrentItem());
    ...
    public void onUIOpen(PlayerOpenUIEvent e){
        if(e.getUIType() != TradingUIType.INSTANCE)
            return;
        if(!Main.getInstance().getConfig().getBoolean("EconomyHook.Geyser-Compat.Enabled")){
            return;
        }
        ...
        (new BukkitRunnable(){
            @Override
            public void run() {
                if(p.getName().indexOf(Main.getInstance().getConfig().getString("EconomyHook.Geyser-Compat.Name-Prefix"))==0){
