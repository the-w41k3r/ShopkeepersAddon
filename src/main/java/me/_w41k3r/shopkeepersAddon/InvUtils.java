...
public class InvUtils {
    ...
    public static ItemStack customPlayerHead(String skin, List<String> lore, String name) {
        ...
        //item = Bukkit.getUnsafe().modifyItemStack(head, dev.MrFlyn.shopkeeperNavAddon.Main.getInstance().getConfig().getString("backpack." + String.valueOf(lev) + ".skin"));
    ...
    public static ItemStack ItemBuilder(Material m, int amount, String name, List<String> lore, String persistentDataKey, Double persistentData) {
        ...
        meta.setDisplayName(name);
        meta.setLore(lore);
        NamespacedKey key = new NamespacedKey(Main.getInstance(), persistentDataKey);
    ...
    public static boolean hasPersistentData(String key, ItemStack i, PersistentDataType type){
        ...
        if(!i.hasItemMeta())
            return false;
        return i.getItemMeta().getPersistentDataContainer().has(new NamespacedKey(Main.getInstance(), key), type);
    }
    ...
    public static double getPersistentDataPrice(ItemStack i) {
        return i.getItemMeta().getPersistentDataContainer().get(new NamespacedKey(Main.getInstance(), "ItemPrice"),PersistentDataType.DOUBLE);
    }
