    ...
    public static void nextPage(Inventory inv, Player p, MenuType mt) {
        switch (mt) {
            ...
            case ITEM_SHOP:
                ...
                List<ItemStack> pageItems = cachedItems.subList(pageNumber * 36, Math.min(cachedItems.size(), (pageNumber + 1) * 36));
                if (pageItems.size() <= 36) {
                    inv.setItem(51, new ItemStack(Material.LIGHT_BLUE_STAINED_GLASS_PANE, 1));
                } else {
                ...
                for (int i = 0; i < 36; i++) {
                    if (i < pageItems.size())
                        ...
                        inv.setItem(i + 9, new ItemStack(Material.BARRIER, 1));
                }
                break;
        }
    }
    ...
    public static void previousPage(Inventory inv, Player p, MenuType mt) {
        switch (mt) {
            ...
            case ITEM_SHOP:
                ...
                ShopInv holder = (ShopInv) inv.getHolder();
                List<ItemStack> cachedItems = holder.getCachedPageItems();
                List<ItemStack> pageItems = cachedItems.subList(Math.max((pageNumber - 2) * 36, 0), (pageNumber - 1) * 36);
                if (pageItems.size() <= 36) {
                    inv.setItem(51, new ItemStack(Material.LIGHT_BLUE_STAINED_GLASS_PANE, 1));
                } else {
                    inv.setItem(51, InvUtils.customPlayerHead(PlayerHeadSkins.NEXT_PAGE.toString(), null, "Next Page"));
            ...
            case REMOTE_ADMIN_SHOP:
                ...
                List<ItemStack> pageItems1 = cachedItems1.subList(Math.max((pageNumber1 - 2) * 36, 0), (pageNumber1 - 1) * 36);
                ...
                if ((pageNumber1 - 2) * 36 == 0) {
                    inv.setItem(47, new ItemStack(Material.LIGHT_BLUE_STAINED_GLASS_PANE, 1));
                } else {
                    inv.setItem(47, InvUtils.customPlayerHead(PlayerHeadSkins.PREVIOUS_PAGE.toString(), null, "Previous Page"));
                }
                for (int i = 0; i < 36; i++) {
